...
# Healthcheck: runs `pnte:diag` (cached, no-audit) and exits 0 if all services ok.
...
HEALTHCHECK --interval=30s --timeout=20s --start-period=60s --retries=3 \
    CMD php /var/www/html/artisan pnte:diag --quiet-ok --no-audit --json > /dev/null 2>&1 || exit 1
