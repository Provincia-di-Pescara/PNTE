...
@section('title', 'Nuovo cantiere')
@section('content')
<div class="mb-6">
    <nav class="text-sm text-slate-500 mb-2">
        <a href="{{ route('third-party.roadworks.index') }}" class="hover:text-slate-700">Cantieri</a>
        <span class="mx-1">/</span><span>Nuovo</span>
    </nav>
    <h1 class="text-xl font-bold text-slate-900">Nuovo cantiere stradale</h1>
</div>
@if($errors->any())
...
<div class="bg-white rounded-xl border border-slate-200 p-6 max-w-2xl">
    <form method="POST" action="{{ route('third-party.roadworks.store') }}">
        @csrf
        @include('third-party.roadworks._form')
        <div class="flex gap-3 mt-8 pt-6 border-t border-slate-100">
            <button type="submit" class="px-5 py-2 rounded-lg bg-blue-600 text-sm font-semibold text-white hover:bg-blue-700 transition-colors">Crea cantiere</button>
