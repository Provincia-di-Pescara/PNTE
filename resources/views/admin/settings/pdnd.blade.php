...
<div class="mb-6 flex items-center gap-3">
    ...
    <div>
        <h1 class="text-xl font-bold tracking-tight">PDND / Interoperabilità</h1>
        <p class="text-sm text-ink-2 mt-0.5">Configurazione PDND Interoperabilità: autenticazione, IPA e Registro Imprese.</p>
    </div>
</div>
...
<div class="space-y-6 max-w-2xl">
    ...
    {{-- ── Autenticazione PDND ────────────────────────────────────────── --}}
    <div class="card p-6">
        <h2 class="text-sm font-semibold mb-4 border-b border-line pb-2">Autenticazione PDND</h2>
    ...
    {{-- ── IPA (AgID) ─────────────────────────────────────────────────── --}}
    <div class="card p-6">
        <h2 class="text-sm font-semibold mb-1 border-b border-line pb-2">IPA — Indice Pubbliche Amministrazioni</h2>
        <p class="text-[12px] text-ink-2 mb-4">Sincronizza le PEC degli enti dal catalogo AgID IPA. L'e-service IPA ha accettazione automatica su PDND.</p>

        <form method="POST" action="{{ route('admin.settings.pdnd.update') }}" class="space-y-4">
            @csrf
            @method('PUT')
            {{-- Preserve other fields unchanged when saving only IPA URL --}}
    ...
    <div class="card p-6">
        <h2 class="text-sm font-semibold mb-1 border-b border-line pb-2">InfoCamere — Registro Imprese</h2>

        {{-- Warning: manual acceptance required --}}
        <div class="my-4 flex gap-3 rounded-md bg-warning/10 border border-warning/30 px-4 py-3">
            ...
            <div class="text-[12px] text-ink">
                ...
                <p>L'e-service <em>Servizi consultazione Registro Imprese</em> (Unioncamere) richiede approvazione manuale da parte di Unioncamere sul portale PDND. Richiedi l'accordo di fruizione su <a href="https://selfcare.pagopa.it" target="_blank" rel="noopener" class="underline font-medium">interop.pagopa.it</a> e attendi l'accettazione prima di configurare l'URL qui sotto.</p>
            </div>
        </div>
