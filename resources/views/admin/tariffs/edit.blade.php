...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold tracking-tight">Modifica coefficiente tariffario</h1>
</div>
...
<form method="POST" action="{{ route('admin.tariffs.update', $tariff) }}" class="max-w-2xl space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="card p-6 space-y-5">
        ...
        <div>
            ...
            <input type="number" id="coefficiente" name="coefficiente" value="{{ old('coefficiente', $tariff->coefficiente) }}" step="0.000001" min="0" required
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" id="valid_from" name="valid_from" value="{{ old('valid_from', $tariff->valid_from->format('Y-m-d')) }}" required
            ...
            <div>
                ...
                <input type="date" id="valid_to" name="valid_to" value="{{ old('valid_to', $tariff->valid_to?->format('Y-m-d')) }}"
        ...
        <div>
            ...
            <textarea id="note" name="note" rows="3"
                      class="w-full p-3 rounded-md border border-line bg-surface text-[13px] focus:border-accent focus:outline-none transition-colors @error('note') border-danger @enderror">{{ old('note', $tariff->note) }}</textarea>
